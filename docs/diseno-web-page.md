...
| 3 | **Problema** | `diseno-web-problem.tsx` | 🆕 propia | 4 cards de dolor. Íconos emoji (placeholder). |
...
| 9 | **CTA** | `mid-cta-section.tsx` | ♻️ reusada | Spotlight + Boxes. |
...
- **Cursor**: la página lleva `.dw-page` en el `<main>` → restaura el puntero nativo (el `body{cursor:none}` global del home dejaba las otras páginas sin cursor).
...
8. **Commits**: TODO sin commitear (home mobile + página nueva). Asegurar en commits conventional antes de seguir.
